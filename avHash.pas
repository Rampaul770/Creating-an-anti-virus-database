...
// вычисление хеш-суммы для файла
function MD5F(const FileName: string): TMD5Digest;

function MD5FileStream: String;

function MD5FileStreamEx: String;
...
begin
  try
    D := MD5Stream(FStream);
    Result := LowerCase(MD5DigestToStr(D) +':'+inttostr(FStream.Size));
  except
  end;
end;
...
begin
  try
    D := MD5Stream(FStreamEx);
    Result := LowerCase(MD5DigestToStr(D) +':'+inttostr(FStreamEx.Size));
  except
  end;
end;
...
begin
  try
    ...
    D := MD5Stream(FS);
    Result := MD5DigestToStr(D) +':'+inttostr(FS.Size);
    TST.Add(FileName+' - '+Result); //Для теста
    TST.SaveToFile('Hash.log');
  finally
    FS.Free;
    TST.Free;
  end;
